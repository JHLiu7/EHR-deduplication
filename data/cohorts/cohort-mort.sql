...
, disch_by_tsf as (
    ...
    from 
        note_time
    where 
        ctime_icu <= 24 or cdate_icu <= 24
)
-- , tmp as (
select 
    iv.*, adm.HOSPITAL_EXPIRE_FLAG MORT_HOSP
from icu_view iv
    inner join note_crit n on iv.HADM_ID = n.HADM_ID
    left join adm_disc_time t on iv.HADM_ID = t.HADM_ID
    left join los l on iv.HADM_ID = l.HADM_ID
    left join age a on iv.HADM_ID = a.HADM_ID
    left join `physionet-data.mimiciii_clinical.admissions` adm on adm.HADM_ID = iv.HADM_ID
where
    a.age > 15 and l.LOS_HOUR_ICU > 24 + 6 # pred window + 24hr
order by 
    iv.SUBJECT_ID, iv.HADM_ID, iv.ICUSTAY_ID

-- ) select count(*) / (select count(*) from tmp) from tmp group by MORT_HOSP

# 33528 queried on 10/18/2021
